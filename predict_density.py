...
parser.add_argument('--data_path', default='./predict_density', type=str, help='path to dataset')
...
with torch.no_grad():
    ...
    for i, (images, gt) in enumerate(test_loader):
        ...
        plt.imshow(predict)
        pathD = './predict_density/density/D_' + str(i) + '_p{:.2f}_l{:.2f}'.format(predict.sum().item(), gt.item()) + '.jpg'
        plt.savefig(fname=pathD)
